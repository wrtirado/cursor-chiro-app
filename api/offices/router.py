...
def update_existing_office(
    ...
    return crud_office.update_office(
        db=db,
        db_office=db_office,
        office_in=office_in,
        current_user_id=current_user.user_id,  # Pass the current user's ID
    )
...
def assign_manager(
    ...
    assigned_office = crud_office.assign_manager_to_office(
        db, db_office=db_office, manager_user_id=request.manager_user_id
    )
    ...
    if assigned_office is None:
        ...
        raise HTTPException(
            ...
            detail="Invalid user ID or user is not an Office Manager",
        )
