from sqlalchemy import Column, Integer, String, ForeignKey, TIMESTAMP, Boolean, JSON, Text
...
class User(Base):
    ...
    assignments_given = relationship("PlanAssignment", foreign_keys="[PlanAssignment.assigned_by_id]", back_populates="assigner")
    assignments_received = relationship("PlanAssignment", foreign_keys="[PlanAssignment.patient_id]", back_populates="patient")

class TherapyPlan(Base):
    __tablename__ = "therapyplans"
    plan_id = Column(Integer, primary_key=True, index=True)
    chiropractor_id = Column(Integer, ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False)
...
class PlanExercise(Base):
    ...
    plan_id = Column(Integer, ForeignKey("therapyplans.plan_id", ondelete="CASCADE"), nullable=False)
    title = Column(String(255), nullable=False)
...
class PlanAssignment(Base):
    ...
    patient = relationship("User", foreign_keys=[patient_id], back_populates="assignments_received")
    assigner = relationship("User", foreign_keys=[assigned_by_id], back_populates="assignments_given")
    progress = relationship("Progress", back_populates="assignment")

class Progress(Base):
    __tablename__ = "progress"
    progress_id = Column(Integer, primary_key=True, index=True)
    assignment_id = Column(Integer, ForeignKey("planassignments.assignment_id", ondelete="CASCADE"), nullable=False)
    plan_exercise_id = Column(Integer, ForeignKey("planexercises.plan_exercise_id", ondelete="CASCADE"), nullable=False)
...
class Branding(Base):
    ...
    office_id = Column(Integer, ForeignKey("offices.office_id", ondelete="CASCADE"), unique=True, nullable=False)
